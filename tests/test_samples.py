...
def test_random_squares():
    ...
    for angle, size, position, color in zip(angles, sizes, positions, colors):
        square = gz.square(
            size, xy=position, angle=angle, fill=color, stroke_width=size / 20
        )
        square.draw(surface)
...
def test_yin_yang():
    ...
    surface = gz.Surface(L, L, bg_color=(0, 0.3, 0.6))  # blue background
...
def test_transparent_colors():
    ...
    colors = [
        (1, 0, 0, 0.4),  # <- Semi-tranparent red (R,G,B, transparency)
        (0, 1, 0, 0.4),  # <- Semi-tranparent green
        (0, 0, 1, 0.4),
    ]  # <- Semi-tranparent blue
    circles = gz.Group(
        [
            gz.circle(radius, xy=center, fill=color, stroke_width=3, stroke=(0, 0, 0))
